...
namespace FormLogin
{
    public partial class Form5 : Form
    {
        ...
        private void sgrade_SelectedIndexChanged_1(object sender, EventArgs e)
        {
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            ...
            else if ((nudHr.Value == 0) && (nudMin.Value < 15))
            {
                ...
                try
                {
                    ...
                    foreach (Control a in flowLayoutPanel1.Controls)
                    {
                        ...
                        if (a is MaterialSkin.Controls.MaterialDivider)
                        {
                            ...
                            con.Close();
                            con.Open();
                            string sqlstatement = "insert into tbl_parts(col_part_type,col_part_points,col_exam_id,col_part_no,col_part_items)values(@type,@points,(select count(*) from tbl_exam),@part_no,@item)";
                            MySqlCommand cmd = new MySqlCommand(sqlstatement, con);
                            cmd.Parameters.AddWithValue("@teacher_id", this.Tag);
                            cmd.Parameters.AddWithValue("@exam_name", txtExamName.Text);
                ...
                try
                {
                    ...
                    con.Close();
                    con.Open();
                    string examid = "select count(*) from tbl_exam";
